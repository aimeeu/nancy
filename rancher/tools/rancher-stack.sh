...
# Usage:
# $ git clone https://github.com/aimeeu/nancy.git
# $ cd nancy/rancher
# $ source rancher-tools.sh
# See below for function-specific usage
#

# Install rancher master server
...
function install_master() {
  ...
  # this will launch the latest development build of rancher, validated thru CI automation framework
  # this mode is non-HA
  # http://rancher.com/docs/rancher/v1.6/en/installing-rancher/installing-server/
  echo "${FUNCNAME[0]}: installing rancher server (master) in non-HA mode with internal database"
